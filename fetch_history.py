...
raw_url = os.environ["ODOO_URL"].strip().rstrip("/")
...
orders = paged("pos.order", [
    ["state", "in", ["done", "invoiced"]], ["date_order", "<=", period_end_s + " 23:59:59"],
], ["date_order", "amount_total", "config_id"])
branch_months = defaultdict(lambda: defaultdict(lambda: {"revenue": 0.0, "transactions": 0}))
for order in orders:
    cfg = order.get("config_id")
    ...
    mk = month_key(order["date_order"])
    branch_months[name][mk]["revenue"] += float(order.get("amount_total") or 0)
    branch_months[name][mk]["transactions"] += 1
